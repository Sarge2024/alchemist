...
export const ModerationService = {
  ...
  async validateCulinaryRelevance(text: string): Promise<'approved' | 'rejected'> {
    ...
    // Caso a chave da API não esteja configurada, o sistema falha com segurança aprovando o conteúdo (fail-open para dev)
    if (!apiKey || apiKey === "" || apiKey === "your_gemini_api_key_here") {
      console.warn("[Moderation] GEMINI_API_KEY não configurada. Aprovando mensagem por padrão.");
      return 'approved';
    }
    ...
    try {
      ...
      console.error("[Moderation Error] Falha na análise do Gemini:", errorMessage);
